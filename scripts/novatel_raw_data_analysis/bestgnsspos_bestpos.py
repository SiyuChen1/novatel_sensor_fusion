...
def plot_bestgnsspos_bestpos(abs_path, file_name):
    ...
    bestgnsspos_ts = []
    bestpos_ts = []
    with open(Path(abs_path) / file_name) as data_file:
...
def search_specific_gps_time(gps_time_array, time_of_week):
    for index in range(len(gps_time_array) - 1):
        if gps_time_array[index].time_of_week <= time_of_week <= gps_time_array[index + 1].time_of_week:
            return index


abs_path = Path(os.path.abspath(__file__))
parser = argparse.ArgumentParser(description="Visualizing bestgnss and gnss positions")
parser.add_argument("--folder-path", type=str,
                    default=str(abs_path.parent.parent.parent / 'data/gnss_imu_logs/'))
parser.add_argument('--file-name', type=str,
                    default='IMUData_20230703_140222.log')
